...
import type { PartialVideos, Videos } from "@/types/vanko.type";
...
export default function ArchiveAdmin({ videos }: ArchiveAdminProps) {
	...
	const setRequestInit = useCallback(
		(ref: HTMLFormElement) => {
			...
			const keyword_1 =
				ref.keyword1.value.length > 0
					? ref.keyword1.value
					: selectedItem?.keywords?.[0];
			const keyword_2 =
				ref.keyword2.value.length > 0
					? ref.keyword2.value
					: selectedItem?.keywords?.[1];
			const keyword_3 =
				ref.keyword3.value.length > 0
					? ref.keyword3.value
					: selectedItem?.keywords?.[2];
			...
			<div style={{ display: "flex", flexDirection: "column", gap: "3rem" }}>
				...
				<div className={styles.leftboxs}>
					<div className={styles.leftboxstitle}>
						<span style={{ backgroundColor: "orange" }}>선택된</span> 아이템
						수정하기
					</div>
						...
						>
							...
							>
								...
								<input
									...
									name="title01"
									placeholder={
										selectedItem ? (selectedItem.title as string) : "선택요망"
									}
									defaultValue={
										selectedItem ? (selectedItem.title as string) : "선택요망"
									}
								...
								<input
									...
									name="embed"
									placeholder={
										selectedItem ? (selectedItem.embed as string) : "선택요망"
									}
									defaultValue={
										selectedItem ? (selectedItem.embed as string) : "선택요망"
									}
								...
								>
									<input
										...
										placeholder={
											selectedItem
												? (selectedItem.keywords?.[0] as string)
												: "선택요망"
										}
										defaultValue={
											selectedItem
									...
									<input
										...
										placeholder={
											selectedItem
												? (selectedItem.keywords?.[1] as string)
												: "선택요망"
										}
										defaultValue={
											selectedItem
									...
									<input
										...
										placeholder={
											selectedItem
												? (selectedItem.keywords?.[2] as string)
												: "선택요망"
										}
										defaultValue={
											selectedItem

...
const taropicture = [
    ...
    },
    {
        ...
        text1: '올해 바다로 여행을 간다면 좋은 일이 생길 운이 있네요.\n기운이 좋은 달은 4월, 9월입니다.',
...
export default function Saju({setSowonCount, closeUnse, mobile, android}){
    ...
    const [pngWebp, setPngWebp] = useState(null);
    const [taroBig, setTaroBig] = useState(false);

    const isSubmittingRef = useRef(false);

    const sowonRef = useRef();
    ...
    const sowonClick = async (e) => {
        e.preventDefault();
        // alert('소원이 제출되었습니다.')
        // if (isSubmittingRef.current) {
        //     return; // 이미 제출 중이면 추가 처리를 하지 않음
        // }
        // isSubmittingRef.current = true;
        // console.log(isSubmittingRef.current)
        ...
        if (isSubmittingRef.current === false) {
            ...
            // finally {
            //     isSubmittingRef.current = false; // 처리가 완료되면 상태를 다시 변경 <<< finally 문 안에다 이거 넣으면 안됨!!! 두번 실행됨 response.ok 에다 넣어야 함
            // }
        }
    ...
    const handleTaro = (item, i) => (e) => {
        ...
        const resultNum = distributing(item.count);
        const toSrc = `${item.src}${resultNum}`;
        setPngWebp(toSrc);
        setFullText(item[`text${resultNum}`])
    }
    ...
    return(
        <>
        {taroBig && pngWebp && <TaroBig setTaroBig = {setTaroBig} pngWebp = {pngWebp} mobile = {mobile} android = {android}/>}
            ...
            <div className={styles.unsebackground}>
                        ...
                        <div className={styles.taro}>
                            {
                                taropicture.map((item, i)=>{
                                    return(
                                        <div onClick={handleTaro(item, i)} className={styles.tarobox} key={i}>
                                            <img src='/assets/img/taro/taro_back.webp' className={styles.tarocard} style={{pointerEvents : select ? 'none' : 'all'}} alt='taroback'></img>
                                            <p>{item.con}</p>
                                        </div>
                                    )
                                })
                ...
                {sowonForm &&
                    (<div className={styles.marqueeinput}>
                        ...
                        <form className={styles.sowonform} ref={sowonRef} onSubmit={sowonClick} disabled={isSubmittingRef.current}>
                            <input className={styles.sowonp} type='text' name='sowoncontents' placeholder='진실된 마음으로... ' disabled={isSubmittingRef.current}></input>
                            <input className={styles.click} type="submit" value="제출하기" disabled={isSubmittingRef.current} style={{pointerEvents: isSubmittingRef.current ? "none" : "all"}}></input>
                        </form>
                    </div>)
                }
                ...
                {load &&
                    (<div className={styles.loadbackground}>
                        <div style={{display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center'}}>
                            <img className={styles.loadimg} src='/assets/gifs/stone.gif' alt='stone'></img>
                ...
                <div className={styles.newYearground} style={{opacity: newYear ? "1" : "0", zIndex: newYear ? "20001" : "-1"}}>
                    <p onClick={closeUnse} className={styles.newx}>X</p>
                    <div className={styles.newyearbox}>
                        {pngWebp && 
                            (<img className={styles.newyearimg} onClick={openTaroBig} src={`${pngWebp}.webp`} alt='taro'></img>)
                        }
...
function TaroBig({setTaroBig, pngWebp, mobile, android}){
    const bigTaroImgRef = useRef();
    const bigTaroDownRef = useRef();
    const bigTaroDownTwoRef = useRef();

    const closeTaroBig = () => {
        setTaroBig(false)
    }
    const clickOutside = (e) => {
        if(e.target !== bigTaroImgRef.current && e.target !== bigTaroDownRef.current && e.target !== bigTaroDownTwoRef.current){
            setTaroBig(false);
        }
    }
    ...
    const shareImageFile = async (imageUrl) => {
    ...
    return(
        <div className={styles.bigtarocontainer} onClick={clickOutside}>
            <div style={{display:'flex', justifyContent:'flex-end'}}>
                <span className={styles.close} onClick={closeTaroBig}>X</span>
            </div>

            <div className={styles.bigtarocenter}>
                <div className={styles.bigtarobox}>
                    <img ref={bigTaroImgRef} className={styles.bigtaroimg} src={`${pngWebp}.webp`}></img>
                </div>
                <div>
                    {mobile && android === false ? 
                        <div style={{textDecoration:'none', display:'flex', justifyContent:'center'}}>
                            <p style={{backgroundColor:'#5d3c37'}} onClick={() => shareImageFile(`${pngWebp}.png`)} className={styles.download} ref={bigTaroDownRef}>download</p>
                        </div>
                    : 
                        (
                            <a href={`${pngWebp}.png`} style={{textDecoration:'none', display:'flex', justifyContent:'center'}} alt='taroimg' target='_blank' rel='noopener noreferrer' download>
                                <p ref={bigTaroDownTwoRef} className={styles.download}>download</p>
                            </a>
                        )}
                </div>
            </div>

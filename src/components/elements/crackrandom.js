...
export default function CrackRandom({width, height, crack}) {
    ...
    function drawComplexCrack(x, y) {
        ...
        for (let i = 0; i < length; i+=5) {
            ...
            if (Math.random() > 0.7) { // 30% 확률로 균열 분기
    ...
    function drawBranchCrack(x, y, length, angle, ctx) {
        // const ctx = canvasRef.current.getContext('2d');
    ...
    return (
        <div style={{position: "absolute", width: width, height: height, zIndex: "10" }}>
            <div className={styles.drawing}>
                <canvas ref={canvasRef} style={{position: "relative", width: width, height: height}} width={width} height={height}></canvas>
            </div> 
        </div>       
    )
}
...
// import { bgmsList } from "./bgmsList.js";
import dayjs from 'dayjs';
import VolumeNob from "./volumeNob.js";
import styles from './page.module.css';
import { silkscreen } from "@/app/layout.js";
// import '../../app/globals.css'

export default function VanPlayer({bgmsList}){

    // const bgmsList = useSelector((state)=>state.bgmsList.value);
    // const audioRef = useContext(AudioContext); 
    // const dispatch = useDispatch();

    const audioRef = useRef(); // 오디오 객체 ref 생성
    ...
    const [mobile, setMobile] = useState(null); // 모바일 체커
    ...
    const handleLoad = () => {
        setLoad(true);
        if(play) audioRef.current.play(); // 만약 곡이 넘어갔을 때, 연속재생 중이면(가능이면) 재생해라
    };
    ...
    const handleEnded = () => {
        const result = bgmsList.filter((e, i)=>{
            if(Number(audioRef.current.dataset.num) === bgmsList.length -1){ // 재생종료시 마지막리스트이면
                return setPlay(false); // 플레이스테이트 false
            }else{
                return i === Number(audioRef.current.dataset.num) + 1 // 마지막리스트가 아니면 인덱스 +1 해서 객체 리턴 
            }   
        });
        result.length > 0 && setSrc(...result); // 리턴받은 값이 있으면, src 스테이트로 설정
    };
    ...
    const handleBefore = () => {
        const result = bgmsList.filter((e, i)=>{
            if(Number(audioRef.current.dataset.num) === 0){ // 재생목록 인덱스 0 이면 그냥 리턴
                return;
            }else{
    ...
    useEffect(()=>{
        if(src){ 
            if(src.publicNum === 0){
                ...
                // setLoad(true);
            }else if(audioRef.current.src === src.firebaseUrl){ // 이전 곡src 와 src 스테이트가 일치하면 === 잘 넘어갔으면
            ...
            <div className={`${styles.music_container} ${play && "play"}`}>
                ...
                {/* {src && 
                    <audio 
                        ...
                        // muted
                        data-name={src.name}
                        data-num={src.publicNum}
                        ref={audioRef} 
                        src={bgmsList[0].firebaseUrl}
                ...
                <div className={styles.nav_wrapper}>
                    <div className={styles.nav_lower_container}>
                        <span className={`${styles.music_time} ${silkscreen.className}`}>{nowM} / {maxM}</span>
                        {src && <span className={`${styles.music_title} ${silkscreen.className}`}>{src.name}</span>}
                        <span className={`${styles.material_symbols_outlined} ${styles.list}`} onClick={handleListOpen}>list</span>
                    </div>
                    ...
                    <div className={styles.nav_upper_container}>
                        ...
                        <div className={styles.navigation_upper}>
                            <span className={`${styles.material_symbols_outlined} ${styles.fast_btn}`} style={{ pointerEvents : load ? "auto" : "none" }} onClick={handleBefore}>fast_rewind</span>
                            { play === false ? 
                                <span className={`${styles.material_symbols_outlined} ${styles.play_btn}`} style={{ pointerEvents : load ? "auto" : "none" }} onClick={handlePlay}>play_arrow</span> : 
                                <span className={`${styles.material_symbols_outlined} ${styles.pause_btn}`} style={{ pointerEvents : load ? "auto" : "none", textShadow: '0px 0px 5px aqua, 0px 0px 5px aqua, 0px 0px 5px aqua'}} onClick={handlePauseClick}>pause</span>
                            }
                            <span className={`${styles.material_symbols_outlined} ${styles.fast_btn}`} style={{ pointerEvents : load ? "auto" : "none" }} onClick={handleNext} >fast_forward</span>
                            <span className={`${styles.material_symbols_outlined} ${styles.replay_btn}`} style={{ pointerEvents : load ? "auto" : "none" }} onClick={handleRewind}>replay</span>

                            <div style={{display:'flex', justifyContent:'center', alignItems:'center'}} onMouseEnter={handleNob} onMouseLeave={handleNob} >
                                {volume > 0 && mute === false ? <span className={`${styles.material_symbols_outlined} ${styles.volume}`} onClick={handleNob} >volume_up</span> : <span style={{ left : mute ? "0px" : "0px", textShadow: mute && '0px 0px 5px aqua, 0px 0px 5px aqua, 0px 0px 5px aqua'}} className={styles.material_symbols_outlined} onClick={handleNob2} >{mute ? volume_off : volume_mute}</span>}
                                {/* {nob && (<VolumeNob volume={volume} setVolume={setVolume} mobile={mobile} /> )} */}
                                <VolumeNob volume={volume} setVolume={setVolume} mobile={mobile} />

                                {/* {mobile && <VolumeNob volume={volume} setVolume={setVolume} mobile={mobile}/>} */}
                            </div>
                        </div>
                    </div>
                ...
                {listOpen && (
                    <div className={styles.music_info}>
                        {bgmsList.length > 0 && bgmsList.map((item, index) => (
                            <div key={item.name+index}><p style={{ color : src.publicNum === index ? "aqua" : "lightblue", pointerEvents : load ? "auto" : "none" , fontFamily:'auto' }} onClick={handleListClick(item)}>{item.name}</p></div>
                        ))}
                    </div>
                )}
            </div>
...
function VanAmpBtn(){
    return(
        <div className={styles.vanampboxex}>
                ...
                <div className={styles.vanampbox}><div className={styles.vanampbtn}>VAN AMP</div></div>

...
export default function VaporwaveScene2d({
	threeD,
	ready,
	audio,
	play,
}: VaporwaveScene2dProps) {
	const [_isLoaded, setIsLoaded] = useState(false); // 이미지 로드 상태를 추적하는 상태 변수
	...
	return (
		// @ts-expect-error - React 19 compatibility issue with react-spring
		<animated.div
			className={styles.canvas2d}
			style={{ opacity: opacity, position: "absolute", zIndex: "1" } as any}
		>
			<img
				ref={earthRef}

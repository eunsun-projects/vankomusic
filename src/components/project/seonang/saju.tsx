...
import type { Wishs } from "@/types/vanko.type";
...
const taropicture = [
	{
		...
		count: 5,
		text0:
			"올해 재물과 큰 복이 함께 들어올 예정이군요.\n그 기회를 놓치지 마시길…!",
		text1:
			"갑자기 나타난 큰 힘이 당신을 도울 것입니다.\n겸손하고 차분하게 올라가세요.",
	...
	},
	{
		...
		count: 3,
		text0:
			"출제 의도를 한 눈에 파악할수 있는 개안부적입니다.\n출력하여 책상 밑에 붙여두세요.  ",
		text1:
			"모두가 선망하는 지식을 손에 넣는 해입니다.\n집중해서 내 것으로 만드세요.",
		text2:
			"집중력이 최고조에 달하여 암기의 왕이 될 것입니다.\n몽땅 외워 버려욧!",
	},
	{
...
export default function Saju({ closeUnse, mobile, android }: SajuProps) {
	...
	const handleTaro =
		(item: (typeof taropicture)[number]) =>
				...
				<div className={styles.unsebackground}>
						...
						<div className={styles.unseimgdiv}>
							...
							<div className={styles.taro}>
								{taropicture.map((item, i) => {
									return (
										<div
											onClick={handleTaro(item)}
											className={styles.tarobox}
											key={i}
										>
											<img
					...
					{sowonForm && (
						<div className={styles.marqueeinput}>
							...
							<form
								className={styles.sowonform}
								ref={sowonRef}
								onSubmit={handleSowonSubmit}
							>
								<input
								...
								<button
									...
									disabled={isSubmittingRef.current}
									style={{
										pointerEvents: isSubmittingRef.current ? "none" : "all",
									}}
								>
									제출하기
								</button>

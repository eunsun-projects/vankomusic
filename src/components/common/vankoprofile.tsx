...
function loadImageToCanvas(url: string, canvas: HTMLCanvasElement): Promise<void> {
...
async function extractColorsFromBMP(url: string): Promise<THREE.Color[]> {
...
const BoxGrid = () => {
  ...
  useEffect(() => {
    extractColorsFromBMP('/assets/img/vanko_profile_pixel.bmp').then((colors) => {
  ...
  return (
    <>
      <directionalLight castShadow color={0xffffff} intensity={8} />
      ...
      {colorArr &&
        colorArr.map((color, index) => {
          ...
          return (
            <group
              key={index}
              position={[-totalWidth / 2 + size / 2, -totalHeight / 2 + size / 2, 0]}
            >
              <mesh

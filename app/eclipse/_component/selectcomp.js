...
import { eclipseNodes } from "../_data/eclipsenodes";
...
export default function SelectComp(){
    ...
    const [conversation, setConversation] = useState(
        [
            ...
            [
                eclipseNodes[0].android[0], 
                eclipseNodes[0].player[0]
            ]
        ]
    );
    ...
    const setDataWhenClick = (e) => {
        ...
        if(type === 'next'){
            ...
            const to = eclipseNodes.find((e,i) => e.number === currentObject.to );
            // 찾은 객체로 setState
            setCurrentObject( to );
    ...
    return(
        <div className={styles.conversationbox}>
            ...
            {conversation[0] && conversation[1].map((e,i)=>(
                <p key={i} style={{ whiteSpace: 'pre-line'}}>{e}</p>
            ))}

            {currentObject.isSelect && (
                <div>
                    <p data-type='one' onClick={setDataWhenClick}>{currentObject.select.one[0]}</p>
                    <p data-type='two' onClick={setDataWhenClick}>{currentObject.select.two[0]}</p>
                    {currentObject.select.three.length > 0 && (<p data-type='three' onClick={setDataWhenClick}>{currentObject.select.three[0]}</p> )}
                    {currentObject.select.four.length > 0 && (<p data-type='four' onClick={setDataWhenClick}>{currentObject.select.four[0]}</p> )}
                    {currentObject.select.five.length > 0 && (<p data-type='five' onClick={setDataWhenClick}>{currentObject.select.five[0]}</p> )}
                </div>
            )}

            {currentObject.isSelect === false && (
                <div data-type='next' onClick={setDataWhenClick}>next</div>
            )}
        </div>
    )
}